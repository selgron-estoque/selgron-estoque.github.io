...
## Acuracidade só conta depois de análise concluída

Cliente perguntou, em cima de um caso real: "tenho itens aguardando
recontagem, itens com divergência e itens aguardando SA [contados em
julho, ainda sem decisão]... a acuracidade desses itens permanecerá
vinculada ao mês de julho... ou passará a compor os indicadores de
agosto?" — respondi o comportamento ATUAL (item pendente já entrava na
média de acuracidade na hora, contando pra julho, mesmo sem veredito) e
o cliente decidiu, direto: **"Acuracidade contar apenas depois de
análise concluída."**

- **`contagemAnaliseConcluida(c)`** (função nova, logo depois de
  `itemAcuracidade`) — `true` quando o item já tem VEREDITO: contagem ao
  vivo com `statusAprovacao` fora de `OPEN_STATUSES` (aprovado
  automaticamente/na 2ª contagem/pelo líder/pela Diretoria), ou linha do
  histórico com `_statusOriginal` em `HISTORICO_STATUS_CONCLUIDOS`
  (OK/Sem Ajuste/Ajustado). Item ainda `aguardando_segunda`/
  `aguardando_analise_lider`/`aguardando_solicitacao_armazem`/
  `aguardando_aprovacao_diretoria`/histórico "Pendente" → `false`.
- **`data` do registro nunca muda** (fixada uma vez em
  `CountStep.finalize`, mesmo já documentado antes) — é isso que resolve
  a pergunta do cliente sem precisar de nenhum campo/lógica nova: o item
  sempre continua contando pro mês/semana em que foi de fato CONTADO
  (julho), nunca "pula" pro mês em que a decisão saiu (agosto) — só o
  MOMENTO em que ele passa a somar na média é que fica adiado até
  concluir, não a data que ele representa.
- **Escopo restrito a MÉDIAS DE ACURACIDADE, deliberadamente** — 3
  pontos de consumo de `itemAcuracidade` ganharam o filtro
  (`.filter(contagemAnaliseConcluida)` antes de `.map(itemAcuracidade)`,
  ou um `return` antecipado dentro do `forEach`):
  - `Home.acumuladoAte` (KPI "Acuracidade do Estoque").
  - `Dashboard`, `itensComAcuracidadeGeral` (card "Acuracidade Geral",
    Resumo da Operação).
  - `computeWeeklyStats`/`computeMonthlyStats` (gráficos "Acuracidade
    Semanal"/"Acuracidade Mensal") — só a soma de
    `somaAcuracidade`/`comAcuracidade` ganhou o filtro; `total` (volume,
    "Contagens na Semana") continua contando QUALQUER evento de
    contagem real, concluído ou não — mesmo critério que já motivou
    incluir "Pendente" em `historicoParaTendencia` desde o início
    ("um item pendente É uma contagem real que já aconteceu naquele
    dia, só ainda sem veredito").
  - `historicoTendenciaComoContagem` (Dashboard, alimenta
    `poolTendencia`) ganhou `_fromHistoricoTendencia:true`/
    `_statusOriginal` (Status cru da planilha, incluindo "Pendente") —
    sem isso `contagemAnaliseConcluida` não teria como diferenciar um
    item de histórico ainda pendente de um já resolvido nesse pool
    específico (o único que inclui "Pendente" de propósito).
- **Deliberadamente NÃO tocado**: "Itens Divergentes" (KPI + painel +
  card "Itens Divergentes" de Resumo da Operação), "Valor Divergente",
  "Top Itens com Maior Divergência", "Divergência por Família/Grupo",
  "Principais Causas de Erro" — nenhum desses é uma média de
  ACURACIDADE, são listas/contagens de PENDÊNCIA (o que ainda precisa de
  decisão) — esconder um item ainda em aberto delas seria mentir sobre
  o tamanho real da fila, contrariando o próprio propósito dessas telas
  (`DivergentItemsPanel`/`SolicitacaoArmazemPanel`/
  `DiretoriaApprovalPanel` já existem justamente pra mostrar isso).
  "Contagens Concluídas Hoje" também não muda — já é, e sempre foi, "toda
  contagem registrada hoje, qualquer status" (decisão anterior já
  documentada), não uma média de acuracidade.
- Testado via harness Node isolado (mesma técnica de sempre — carrega o
  `index.html` inteiro transpilado numa `vm.Script`): `contagemAnaliseConcluida`
  nos 12 cenários (todo status ao vivo aberto/concluído, histórico
  concluído/`Pendente` via os dois formatos `_fromHistorico`/
  `_fromHistoricoTendencia`); `computeWeeklyStats`/`computeMonthlyStats`
  com o cenário exato do cliente (1 item concluído + 1 pendente na mesma
  semana de julho) — `total` conta os 2, `acuracidade` reflete só o
  concluído (100%, não 75%); o mesmo item, resolvido depois (ainda com
  `data` de julho), passa a contar em julho — nunca pula pra agosto;
  semana 100% sem item concluído vira `acuracidade:null` (não 0%, mesmo
  critério de sempre pra "sem dado" vs. "semana ruim"). Confirmado
  também via render de verdade (jsdom+react-dom/client+`act()`) de
  `Home`/`Dashboard`: o KPI "Acuracidade do Estoque" e o card
  "Acuracidade Geral" mostram 100% (não 75%) com o item pendente
  presente, e "Itens Divergentes" continua contando esse mesmo item
  normalmente (sem regressão nessa métrica, que não é acuracidade).
  Rodada a suíte de regressão completa do scratchpad (338 asserções,
  0 falhas relacionadas — as 2 únicas falhas remanescentes,
  `harness_actions_beside_content`/`harness_dashboard_render_dedup`, já
  existiam de forma idêntica ANTES desta mudança, confirmado via `git
  stash`). Transpile Babel do arquivo inteiro e balanceamento de chaves
  do CSS conferidos (668/668, sem mudança — CSS não foi tocado, só JS).
  **Verificação visual de ponta a ponta em produção fica a cargo do
  cliente** — mesma limitação de sempre (login exige Supabase Auth
  real, não simulável no sandbox sem rede).
